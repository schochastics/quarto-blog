...
Once upon a time I was a dedicated network scientist. These days though, they are more peripheral in my work and I just like to toy around with interesting datasets. One of those is the CRAN *co-authorship network*. 
In such a network, two individuals (in this case R developers), are connected, if they authored a piece of work together. Here, a piece of work is an R package. This network can be easily assembled based on the authors field in all DESCRIPTION files of packages available on CRAN. I have done a low level analysis on [GitHub](https://github.com/schochastics/CRAN_collaboration), also featured in
[tidytuesday](https://github.com/rfordatascience/tidytuesday/tree/master/data/2023/2023-09-19), including the introduction of the **Hadley number**, but I always wanted to do a longer write up. And voila, this is the write up.
...
The actual hard part is to clean up the authors field, because while there exists some standard forms, there is still a lot of free text going on. I tried to to this semi-automatically with a [script](https://github.com/schochastics/CRAN_collaboration/blob/main/Rscripts/helpers.R) which was very tideous and I am sure it is not perfect.
...
The code below is used to build the co-authorship network as a weighted network. The weight indicates, how many packages two developers have authored together.
...
Thats quite a big number of components, which is not necessarily surprising. Many package authors (or teams of authors) have only ever worked on one package and thus never interacted with the broader R developer community.
...
![The biggest Component of the CRAN co-authorship network](https://github.com/schochastics/CRAN_collaboration/raw/main/figures/network.png)

On average, every developer has `r mean(degree(auth_auth_net_largest))` co-authors. The median is `r median(degree(auth_auth_net_largest))`. The person with the most co-authors, namely `r max(degree(auth_auth_net_largest))`, is actually `r V(auth_auth_net_largest)$name[which.max(degree(auth_auth_net_largest))]`. What a great transition for the next section.
...
then you know where this is going. Erdős was working on
solutions to complex mathematical problems with a large number of co-authors by travelling the world and he published more than 1500 papers during his lifetime.
...
but not Erdős himself have an Erdős number of 2, and so on.
The "Hadley number" is defined as the
distance of R developers to Hadley Wickham in the co-authorship network.[^1]
Someone (“A”) who coauthored a package that Hadley is a coauthor of
has a Hadley number of 1. Someone who coauthored a package that A
has coauthored but not Hadley has Hadley number 2, and so on. Hadley
...
To check your own Hadley number (if you are in the largest connected component, and my cleaning script didn't butcher your name), scroll to the end of this post.
...
[^1]: I love this type of numbers. On this blog I also introduced the [Zlatan number](http://blog.schochastics.net/posts/2018-09-28_six-degrees-of-zlatan-ibrahimovic/). 