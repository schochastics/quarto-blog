...
Thats quite a big number of components, which is not necessarily surprising. Many package authors (or teams of authors) have only ever worked on one package (more than 40% of all packages are actually single-authored) and thus never interacted with the broader R developer community on any other package. 
...
On average, every developer has `r round(mean(degree(auth_auth_net_largest)),2)` co-authors. The median is `r median(degree(auth_auth_net_largest))`. 
...
An interesting question one can ask here is who the center of the network is. That is, the person who has the smallest average distance to all other developers. The top ten developers in that regard are shown below. The full list can again be explored at the end of this post.
...
In the below table, you can search for your own Hadley number and where you rank in terms of centrality. 
