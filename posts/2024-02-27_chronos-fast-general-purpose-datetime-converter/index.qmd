...
The package [fasttime](https://github.com/s-u/fasttime) can be used together with chronos to convert larger sets of datetimes by letting chronos return a character vector which is then parsed by `fastPOSIXct` or `fastDate`.
```{r}
#| label: fast_chronos
